...
#' @param input.data.filename name of the file containing the data without the directory name
#' @param num.timepts number of timepoints
#' @param true.net.filename File containing the true network without the directory name
#' @param input.wt.data.filename File containing Input weighted data without the directory name
#' @param is.discrete whether the data is discretized or not
#' @param num.discr.levels number of discreteized levels
...
#' @param mi.estimator The mi estimator to use
#' @param apply.aracne Whether to apply aracne
#' @param clr.algo CLR algo to use Posible values :- {CLR, CLR2, CLR2.1, CLR3, spearman}
#' @param max.fanin Maximum number of input edges in the graph per node
                      ...
                      )
{
  ...
  input.data.filename <- paste(input.dirname, input.data.filename, sep = '/')
  if (true.net.filename != '')
  {
    true.net.filename <- paste(input.dirname, true.net.filename, sep = '/')
  }
  if (input.wt.data.filename != '')
  {
    input.wt.data.filename <- paste(input.dirname, input.wt.data.filename, sep = '/')
  }
